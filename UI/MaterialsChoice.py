...
# Декоратор @dataclass для класса Syrio
@dataclass
class Syrio:
    name: str
    percentage: float
    density: float
    color: Optional[QtCore.Qt.GlobalColor] = None

    def to_dict(self):
        # Преобразуем объект Syrio в словарь для сериализации
        return {
            'name': self.name,
    ...
    @staticmethod
    def from_dict(data):
        # Создаём объект Syrio из словаря (для десериализации)
        return Syrio(
            name=data['name'],
